...
def intersect(edge, planeVertices):
    global planeNormal
...
newVertexId = 0
newVerticesMap = {}
slicingPlane = bpy.data.objects["Slicing plane"]
planeVertices = slicingPlane.data.vertices
newPlaneVertices = []
for i in range(len(planeVertices)):
    newPlaneVertices.append(transform(planeVertices[i].co, slicingPlane, False))
...
for polygon in object.data.polygons:
    ...
    for edgeKey in polygon.edge_keys:
        if edgeKey not in edgeCacheDict:
            ...
            newVertex = intersect(edge, newPlaneVertices)
            if newVertex != None:
    ...
    if addToNewPolygons:
        ...
        while pointer != newVerticesMap[newVertices[3]][1]:
            for edge in vertexEdgeDict[pointer]:
                otherVert = otherVertex(edge, pointer)
                print(str(list(edge.vertices)) + ', ' + str(list(polygon.vertices)))
                if otherVert in polygon.vertices and edge not in edgeSet:
                    pointer = otherVert
                    bottomVertices.append(pointer)
                    edgeSet.add(edge)
